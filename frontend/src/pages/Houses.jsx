...
function Houses() {
  ...
  const categories = ["ALL PROPERTIES", "PRIME ELITE", "LUXURY AREAS", "MID-TIER", "BUDGET / ENTRY"];
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      setLoading(true);
      try {
        // Targeted fetch for house properties matching purpose
        // Increasing limit to 3000 to ensure we get properties for multiple cities
        const { data } = await searchProperties({ type: "House", purpose: purpose, limit: 3000 });
  ...
  // Re-run filtering whenever global filter state changes 
  useEffect(() => {
  ...
  const applyFilters = (data, currPurpose, currCategory, currSize) => {
    ...
    // DB structural filter to strictly ensure only Houses are shown
    result = result.filter(p => p.type === "House");
    ...
    // 5. Category (Price Logic) 
    // This is heuristic based on Elite Horizon's typical pricing
    if (currCategory !== "ALL PROPERTIES") {
      result = result.filter(p => {
        const price = p.price || 0;
        
        if (currPurpose === "RENT") {
          // Rent Specific Thresholds
          switch (currCategory) {
            case "PRIME ELITE": return price >= 200000; // 2 Lac+
            case "LUXURY AREAS": return price >= 100000 && price < 200000; // 1 - 2 Lac
            case "MID-TIER": return price >= 50000 && price < 100000; // 50k - 1 Lac
            case "BUDGET / ENTRY": return price < 50000; // Under 50k
            default: return true;
          }
        } else {
          // Buy Specific Thresholds
          switch (currCategory) {
            case "PRIME ELITE": return price >= 50000000; // 5+ Crore
            case "LUXURY AREAS": return price >= 20000000 && price < 50000000; // 2 - 5 Crore
            case "MID-TIER": return price >= 8000000 && price < 20000000; // 80 Lac - 2 Crore
            case "BUDGET / ENTRY": return price < 8000000; // Under 80 Lac
            default: return true;
          }
        }
      });
  ...
  return (
    <div className="houses-page-wrapper">
      {/* ---- Top Filter Header ---- */}
        ...
        {loading ? (
          ...
          targetCities.map((cityName) => {
            ...
            const enrichedCityProperties = cityProperties.map(p => {
              const price = p.price || 0;
              let mapCat = "Budget"; // Default Yellow
              if (price >= 50000000) mapCat = "Prime"; // Red
              else if (price >= 20000000) mapCat = "Luxury"; // Blue
              else if (price >= 8000000) mapCat = "Mid"; // Green
              return { ...p, category: mapCat };
            });
            ...
            return (
              <div key={cityName} className="city-portal-section">
                ...
                <div className="city-header-box">
                  ...
                  <span className="city-meta-data">{cityProperties.length} Houses Available</span>
                </div>
                  ...
                  {cityProperties.length > 0 ? (
                    <>
                      <div className="responsive-property-grid">
                        {paginatedHouseSlice.map((property, index) => (
                          <PropertyCard
                            key={index}
