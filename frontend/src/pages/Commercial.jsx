...
function Commercial() {
  ...
  const categories = ["ALL PROPERTIES", "OFFICE", "SHOPS", "FACTORY", "WAREHOUSE"];
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      setLoading(true);
      try {
        // Targeted fetch for commercial properties matching purpose
        // Increasing limit to 3000 to ensure we get properties for multiple cities
        const { data } = await searchProperties({ type: "Commercial", purpose: purpose, limit: 3000 });
  ...
  const applyFilters = (data, currPurpose, currCategory, currSize) => {
    ...
    // DB structural filter to strictly ensure only Commercial properties are shown
    result = result.filter(p => p.type === "Commercial");
    ...
    // 5. Category (Commercial Types) 
    // Since the database primarily stores them as generically "Commercial", 
    ...
    if (currCategory !== "ALL PROPERTIES") {
      result = result.filter(p => {
        ...
        const cats = ["OFFICE", "SHOPS", "FACTORY", "WAREHOUSE"];
  ...
  return (
    <div className="commercial-page-wrapper houses-page-wrapper">
      {/* ---- Top Filter Header ---- */}
        ...
        {loading ? (
          ...
          targetCities.map((cityName) => {
            ...
            const enrichedCityProperties = cityProperties.map(p => {
              ...
              // Map commercial types to generic map color categories (to reuse CityMap logic)
              let mapCat = "Budget"; // Default Yellow (Warehouse)
              if (mockCategory === "FACTORY") mapCat = "Prime"; // Red
              else if (mockCategory === "OFFICE") mapCat = "Luxury"; // Blue
              else if (mockCategory === "SHOPS") mapCat = "Mid"; // Green

              return { ...p, category: mapCat, commercialType: mockCategory };
            });
            ...
            return (
              <div key={cityName} className="city-portal-section">
                ...
                <div className="city-header-box">
                  ...
                  <span className="city-meta-data">{cityProperties.length} Commercial Properties Available</span>
                </div>
